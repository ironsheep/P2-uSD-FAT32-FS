...
- **Regression Tested**: 590 automated tests across 28 test suites, certified on two cluster geometries
...
| **[Case Study: The Card That Would Not Start](DOCs/Research/SD-CARD-WEDGE-CASE-STUDY.md)** | Full root-cause study of cards wedged by boot-time traffic on shared pins — evidence, the theories that were wrong, and the one-command fix |
| **[Case Study: What a Socket Costs](DOCs/Research/SD-SOCKET-TIMING-CASE-STUDY.md)** | How two microSD sockets on one board were measured against each other — the read and write phase maps that came out of it, and the driver defaults they changed |
...
| **[Regression Testing](src/regression-tests/README.md)** | Test infrastructure, 590 tests across 28 suites (as of v1.8.0) |
...
> **Socket timing:** Two sockets wired to the same P2 can differ measurably in signal timing — on our bench, an external header-wired adapter adds a ~3 ns-class round-trip delay over the P2 Edge module's onboard socket. At the standard 25 MHz both carry wide margin, but a card that is itself timing-marginal may misbehave in one socket and work in the other. If you see socket-dependent card behavior, see [Socket Timing Differences](DOCs/SD-CARD-PERFORMANCE.md#8-socket-timing-differences) and the *Receive Alignment and Socket Timing* section of the [Theory of Operations](DOCs/SD-CARD-DRIVER-THEORY.md); the full characterization — how the two sockets were measured, the read and write phase maps, and the recipe for measuring your own board — is in [Case Study: What a Socket Costs](DOCs/Research/SD-SOCKET-TIMING-CASE-STUDY.md). Since v1.8.0 the read-path alignment sits at the centre of its measured passing band rather than the lower edge, which equalises read margin between sockets. The one socket-dependent failure we had under investigation turned out not to be a timing effect at all: it was a card left mid-transfer by the boot sequence on shared flash/microSD pins, and v1.8.0 fixes it at card initialisation. The full investigation is written up in [Case Study: The Card That Would Not Start](DOCs/Research/SD-CARD-WEDGE-CASE-STUDY.md).
...
│   └── regression-tests/          # Regression test suite (590 tests as of v1.8.0)
│       ├── SD_RT_*_tests.spin2        # 28 test files (mount, file ops, seek, fatchain, write integrity, async, defrag, etc.)
